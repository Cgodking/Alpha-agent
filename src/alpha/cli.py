...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="alpha", description="Conservative WQB alpha automation service")
    parser.add_argument("--db", default=None, help="SQLite database path")
    parser.add_argument("--env-file", default=".env", help="Optional dotenv-style config file")
    parser.add_argument("--log-file", default="logs/alpha.log", help="Structured service log path")
    sub = parser.add_subparsers(dest="command", required=True)

    sub.add_parser("init-db", help="Initialize database")

    run_once = sub.add_parser("run-once", help="Run one generation/simulation/evaluation cycle")
    run_once.add_argument("--batch-size", type=int, default=None)
    _add_scope_args(run_once)

    daemon = sub.add_parser("daemon", help="Run continuously until interrupted")
    daemon.add_argument("--batch-size", type=int, default=None)
    daemon.add_argument("--loop-seconds", type=float, default=None)
    daemon.add_argument("--run-minutes", type=float, default=None, help="Stop daemon automatically after this many minutes")
    daemon.add_argument("--scope-json", default=None, help="JSON list of scopes to rotate across daemon cycles")
    _add_scope_args(daemon)

    sub.add_parser("status", help="Print candidate status counts")
    sub.add_parser("presets", help="Print available exploration scope presets")
    sub.add_parser("submit-approved", help="Reserved for live submit adapter")
    check_ai = sub.add_parser("check-ai", help="Generate one AI candidate without BRAIN simulation")
    _add_scope_args(check_ai)
    fields = sub.add_parser("fields", help="Print the discovered datafield pool for a scope")
    fields.add_argument("--limit", type=int, default=40)
    _add_scope_args(fields)
    web = sub.add_parser("web", help="Run the local web control panel")
